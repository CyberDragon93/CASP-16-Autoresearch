...
Latest live checkpoint, `2026-07-07 13:20 CDT`: P25 is still not merge-ready.
`finish_p25_scoreable_input_repair.sh --dry-run` reports `ready=false`,
`compatible=true`, `1013` observed candidates, `1037` shard-level candidates
missing, and `967` full 25-candidate slots missing. Slurm still has 19 P25
...
Current live P25 status, checked `2026-07-07 13:20 CDT`: P17 overlay is merged
...
`compatible=true`, with 1013 observed candidates, 1037 shard-level candidates
missing, and 967 full merged candidate slots still missing. Shard05
