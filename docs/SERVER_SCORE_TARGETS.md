...
Latest live checkpoint, `2026-07-07 15:17 CDT`: P25 is still not merge-ready.
`finish_p25_scoreable_input_repair.sh --dry-run` reports `ready=false`,
`compatible=true`, `1331` observed candidates, `727` shard-level candidates
missing, and `657` full 25-candidate slots missing. Slurm still has 19 P25
...
Current live P25 status, checked `2026-07-07 15:17 CDT`: P17 overlay is merged
...
`compatible=true`, with 1331 observed candidates, 727 shard-level candidates
missing, and 657 full merged candidate slots still missing. Shard05
