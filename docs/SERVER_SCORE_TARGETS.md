...
Latest live checkpoint, `2026-07-07 13:13 CDT`: P25 is still not merge-ready.
`finish_p25_scoreable_input_repair.sh --dry-run` reports `ready=false`,
`compatible=true`, `996` observed candidates, `1054` shard-level candidates
missing, and `984` full 25-candidate slots missing. Slurm still has 19 P25
...
silent failure class. The latest error keyword scan is clean, and
...
Current live P25 status, checked `2026-07-07 13:13 CDT`: P17 overlay is merged
...
`compatible=true`, with 996 observed candidates, 1054 shard-level candidates
missing, and 984 full merged candidate slots still missing. Shard05
