...
Latest live checkpoint, `2026-07-07 11:42 CDT`: P25 is still not merge-ready.
`check-shards` reports `ready=false`, `compatible=true`, `751` observed
candidates, `1299` shard-level candidates missing, and `1224` full
...
seed blocks remain zero-output. Error scanning is clean. Do not use a partial
P25 row for score comparisons or for launching O5b/P27b.
...
Current live P25 status, checked `2026-07-07 11:42 CDT`: P17 overlay is merged
...
`compatible=true`, with 751 observed candidates, 1299 shard-level candidates
missing, and 1224 full merged candidate slots still missing. Shard05
