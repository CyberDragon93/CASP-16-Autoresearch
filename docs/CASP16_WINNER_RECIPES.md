...
For the short branch-selection board, read
`docs/WINNER_REPRODUCTION_BOARD.md` first. This file keeps the longer recipe
...
- CASP16 official group registry:
...
- MULTICOM4 CASP16 paper:
  https://www.nature.com/articles/s42003-025-08960-6
...
| Rank | Group | Official name | Fixed mean | Submitted | Missing | Metric |
| ---: | --- | --- | ---: | ---: | ---: | --- |
| 1 | `110s` | MIEnsembles-Server | `0.923321` | 71 | 0 | GDT_TS |
| 2 | `019s` | Zheng-Server | `0.908993` | 71 | 0 | GDT_TS |
| 3 | `147s` | Zheng-Multimer | `0.907055` | 71 | 0 | GDT_TS |
| 4 | `148s` | Guijunlab-Complex | `0.902406` | 71 | 0 | GDT_TS |
| 5 | `456s` | Yang-Multimer | `0.901276` | 69 | 2 | GDT_TS |

Top protein-oligo server groups, fixed mean over 104 official oligo targets:

| Rank | Group | Official name | Fixed mean | Submitted | Missing | Metric |
| ---: | --- | --- | ---: | ---: | ---: | --- |
| 1 | `456s` | Yang-Multimer | `0.582615` | 102 | 2 | QSglob |
| 2 | `052s` | Yang-Server | `0.581712` | 102 | 2 | QSglob |
| 3 | `110s` | MIEnsembles-Server | `0.543327` | 104 | 0 | QSglob |
| 4 | `028s` | NKRNA-s | `0.537240` | 104 | 0 | QSglob |
| 5 | `147s` | Zheng-Multimer | `0.531269` | 104 | 0 | QSglob |
...
- The top-five server set is concentrated in MIEnsembles/Zheng/Yang/Guijun
  system families. The local reproduction priority should therefore stay on
  broad automatic coverage, input realism, candidate diversity, and QA/model
  selection rather than one-off target rescues.
...
## Post-P25 Diversity Branch

P14 and P17 already proved that the repaired scoreable input stack is the
current useful baseline. P25 is now the active gate: do not choose a new GPU
branch from a partial P25 row. After the complete P25 score exists, the next
serious winner-recipe branch should not be "more of the same seed" unless the
evidence points to candidate selection as the bottleneck. Public CASP16 clues
from MULTICOM4, GuijunLab/QA4, and complex assessment instead point at diverse
MSA/model generation plus stronger model quality assessment when seed scaling
is valid but flat.

Predeclared branch name:
...
P27b,
`casp16_server_attack_protenix5_input_repair_defaultparams_model_variant`,
retargets the same default-params probe onto the repaired P17/P25 79-job
scoreable input. It is prepared and preflight-clean, but remains deferred until
P25 finishes and shows that model/config diversity is a better next spend than
more seed scaling or reference recovery.

MSA-mode audit, `2026-07-07 15:37 CDT`: Protenix has a real
`--msa_server_mode protenix|colabfold` flag, and this repo now records
`msa_server_mode` in `run_spec.json` and the generated command. This gives the
future true-MSA branch a concrete production path: compare the default Protenix
MSA service with a global ColabFold/MMseqs MSA mode or precomputed compatible
A3M paths, while keeping `use_msa=true` and `use_template=true`. Do not replace
this with a no-MSA speed row. The local environment also needs an explicit
Protenix-Insta import path at runtime because the bare console script can be
shadowed by OpenDDE's `runner.batch_inference`.

Current live gate, `2026-07-07 18:14 CDT`: P25 is healthy but incomplete.
`scripts/finish_p25_scoreable_input_repair.sh --dry-run` reports
`ready=false`, `compatible=true`, `1725` observed candidates, `345`
shard-level candidates missing, `312` full 25-candidate slots missing, and
`50/79` full-budget tasks complete. Slurm has all remaining P25 shards running
and the CPU closeout watcher `814295` active on `gg`; `zero_output_shard_count=0`.
P25 MSA preflight remains clean, artifact writes reached 18:03 CDT, and the P25
error logs are empty. The current wait is large-complex inference, not an
MSA-cache problem or queue-pending state. This is forward progress, not evidence
for a new strategy choice. Keep O5b, P27b, D6a, and P15/v5 refmap work gated
until the full declared P25 row is merged and scored.
...
- benchmark: `casp16_server_protein_v2_aliasfix` or newer explicit server
  benchmark selected before launch
- input set: start from the same scoreable size-first phase-alias stack unless
  P14 proves that stack is broken
- candidates per target: `4` target-agnostic MSA/model variants times `5`
  fixed seeds, for `20` declared candidates
- MSA policy: real MSA on every variant; no no-MSA toy rows
...
Allowed variant classes:

- existing Protenix full-MSA/template stack as the control variant
- alternative MSA construction from different search tools/databases or
  coverage/e-value cutoffs, recorded as a global recipe rather than per-target
  hand tuning
- domain-aware MSA construction for long/multidomain proteins using only
  benchmark metadata and public sequence annotations
- a second AF3-like backend such as OpenDDE/Protenix variant, if it can run
  with production MSA/template settings and the same input/selection rules

Stop conditions:

- P25 still has `partial_candidates`: wait or repair the execution layer; do
  not score partial shards.
- P25 has many `missing_prediction`, metric failures, or exact-artifact lookup
  failures after completion: fix the pipeline first.
- P25 is mainly reference-limited: launch the v4/P15 row or continue versioned
  reference recovery instead of spending more GPU.
- P25 has broad nonzero signal and improves over P17: analyze target deltas
  before opening another branch.
- P25 is weak despite valid complete predictions: build or launch the prepared
  diversity branch rather than spending a larger single-input seed grid.
...
| G2 | CASP16 winners/top groups used more than one generated model, but ranking was still a bottleneck | `protenix5` and repaired `protenix25_scoreable_input_repair` budgets with `protenix_confidence_v1` | repaired `protenix25` seed106-125 shards were submitted as Slurm jobs `812935..812958` after `24/24 ok` preflight; latest documented live gate at `2026-07-07 18:14 CDT` is healthy but incomplete at `1725` observed candidates, `50/79` full-budget tasks complete, all remaining P25 shards running, and `zero_output_shard_count=0`; active runtime is large-complex inference rather than MSA | Wait for all P25 jobs, merge with the P17 overlay, then score/leaderboard the complete 25-candidate row; keep O5b/P27b/D6a gated until this score exists |
...
| G7 | Reference gaps hide local progress but are not prediction tricks | versioned `refmap` overlays, oligo assembly audit, and Lane C input-alias separation | v4 adds only audited `T1278/T2278`; `diagnostics/reference_gap/t1228v1_v5_refmap_audit.tsv` keeps `T1228V1` blocked on native-state provenance despite full-domain-support candidates; Lane B deferred hits are reviewed and rejected for refmap promotion; Lane D H0217/H0267 oligo candidate assemblies fail target-stoichiometry matching; Lane E relaxed90 domain sweep covers all 16 queued families and rejects the only T0257/T1257/T2257 short tail-fiber template hits; `diagnostics/reference_gap/casp16_server_protein_v5_input_alias_repair_candidates.tsv` records that `T1228V2/T1294V2` are input-repair ready but not reference-ready, while `H1265_V1/V2/V3` now have sequence-level H1265 A/B input repair and complete MSA coverage; all Lane F relaxed90 oligo probes are exhausted across 13 families/35 target rows with only unverified short P13390 tail-fiber domain/template hits and no accepted references | Keep reference recovery opportunistic and versioned; repair realistic inputs where possible, but do not promote references without native/domain or assembly/QSglob proof |
| G8 | QA4/MULTICOM-style systems use diverse MSA/model pools plus QA rather than only seed count | `casp16_server_attack_msa_model_diversity_v1` now uses the repaired P17/P25 input as its base, records the configured P25 consensus replay, and names P27b as the prepared child | P27b prepared/deferred behind P25; true MSA-variant generation is not queued | Build or launch only if P25 is complete/valid but weak and model/config diversity is the recorded next spend |

Post-P25 branch discipline:

- P17 fixed the scoreable missing predictions and scored as a
  `candidate_limited_signal`, so P25 seed106-125 is now submitted. Do not
  launch P27a, P27b, O5/O5b, D6a, or a refmap compute branch until the P25 row
  finishes or fails with a recorded reason.
- If P25 finishes and improves broadly, the next action is analysis, not
  immediate launch. Record target deltas, ok/missing/failed counts, and whether
  the selector chose different seeds for successful targets.
- If P25 is flat with valid predictions, use P27b before another seed grid:
  it is the smallest prepared test of the MULTICOM/QA4 model-config diversity
  clue on the repaired input.
- If P25 exposes domain input-kind or alias zeros, use D6a before more attack
  compute: D6a is already MSA-cache complete and repairs that class.
- If P25 exposes antibody/Fv oligo weakness while non-antibody QSglob rows have
  signal, use O5b before broader oligo tinkering.
- If P25 is capped mostly by `missing_reference`, do versioned reference
  recovery. Do not burn GPU to chase targets that cannot be scored locally.

Default branch after complete P25:

- P25 improves over P17: inspect deltas before deciding whether to scale budget
  or strengthen QA.
- P25 flat but valid: run P27b default-params model/config shards.
- P25 domain zeros from input modality/aliasing: run D6a single-seed input
  repair after marking it pending.
- P25 antibody/Fv QSglob weakness: run repaired O5b shards.
- P25 reference-capped: run P15/v4 only for the v4 benchmark or continue v5
  accepted-reference work; never patch v2/v4 in place.

`docs/SERVER_SCORE_TARGETS.md` owns the exact score targets and P25 closeout
sequence. This file owns the recipe rationale.
...
| D9 | MSA/model diversity budget | MULTICOM4 and QA4-style CASP16 clues emphasize diverse MSA generation, model sampling, and QA over a single default input | design `casp16_server_attack_msa_model_diversity_v1`: repaired P17/P25 input base, 4 target-agnostic MSA/model variants times 5 fixed seeds, real MSA/template settings, no-GPU P25 consensus replay configured, and repaired-input P27b prepared as the first model/config child | build true MSA variants only after P25 proves complete valid predictions are weak and P27b shows model/config diversity is worth scaling |
...
| O5 | Antibody docking branch | kozakovvajda did especially well on antibody-antigen targets without AFM/AF3 as the core engine | `yang_antibody_fv_cleanup_v1` completed a full-set run; `targetlab_protenix_yang_antibody_fv_seed101` completed as a target_lab Fv-only diagnostic with DockQ positives H0233=0.916 and H1233=0.891; O5b now retargets the Fv cleanup onto the repaired 79-job scoreable input | launch O5b only after P25 proves antibody/Fv oligos are the next bottleneck; do not use target_lab DockQ as a server score |
...
17. `casp16_server_attack_protenix25_scoreable_nofail`: prepared but not queued.
    ...
    `casp16_server_attack_protenix25_scoreable_input_repair` is submitted. It
    is the current winner-scale 25-seed successor to P17. The target+seed shard
    manifest points at the repaired 79-target input, reuses the P14 plus
    added-only P17 overlay for seed101-105, and submitted 24 seed106-125 run
    specs as Slurm jobs `812935..812958` after `24/24 ok` preflight with
    complete MSA reuse.
...
37. `casp16_server_attack_msa_model_diversity_v1`: documented as the next
    winner-recipe design gate if P14 is complete and valid but broadly weak.
    It should reproduce the public MSA-diversity plus QA lesson without turning
    off MSA, without per-target tuning, and without pretending a new backend or
    MSA variant is free. Do not queue it until P14 has a fixed-set score and
    the concrete variant inputs can be generated and preflighted.
