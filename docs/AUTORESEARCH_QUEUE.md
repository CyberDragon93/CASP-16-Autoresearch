...
| --- | --- | --- | --- | --- | --- |
| P0 | Validate OpenStructure `ost` QSglob mapping | `casp16_server_protein_v2_aliasfix` | scorer installed; six-target probe has nonzero signal; permissive `--chem-map-seqid-thresh 0` did not rescue H0220/T0234O/H1232 false-zero classes | Required to compare the 104 official server oligo targets and judge antibody-complex strategies | Do not change scorer defaults yet; prioritize v2 protein-oligo input recovery and target-agnostic assembly handling, then rerun `./casp16 score` after active runs finish |
| P1 | Run `server_attack_protenix_terminal_tag_seed101_105` | separate from `dev_fixed` | Slurm job `810719` running; `2026-07-06 16:32 CDT` check had 98 `seed_101` CIFs, 89 `seed_102` CIFs, and 0 for `seed_103..105` | Winner-level server comparison should not pretend one seed/sample is enough | Monitor GH200 job, then score with `protenix_confidence_v1` only after all declared seeds are present; partial rows stay unranked |
...
| P13 | `server_v2_attack_oligo_recovery_nofail_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` pending on dependency `afterany:810938`; token-safe and cache-ready | Stronger five-candidate no-over-token attack with exact-sequence MSA reuse: fixes protein-oligo sequence modality, restores token-safe stoich, applies low-complexity cleanup and fallback, 165 jobs, 0 over-token, 268/268 protein-chain MSA paths reused | It will call `run-next` only after the active v2 dev job clears; score the dev row first if possible, then let the attack run if queue state still supports it |
| P14 | `server_v2_protenix_yang_oligo_sequence_stoich_hydrophobic_leader_nofail_msa_reuse_seed101` | `casp16_server_protein_v2_aliasfix` | Slurm job `811754` pending on dependency `afterany:811751` | Narrow D1d construct-cleanup ablation on the current nofail stack: trims signal-like hydrophobic leaders on 8 T0240/T1210/T1240 alias jobs, keeps 165 jobs and 0 over-token, reuses 260/268 MSA paths | It will call `run-next` after the MSA-reuse attack exits; keep as an ablation and compare only after the main nofail dev/attack rows are scored |
...
| P19 | `casp16_server_attack_protenix25_nofail` | `casp16_server_protein_v2_aliasfix` | planned, not queued; shard manifest points at MSA-reuse input | Same 25-seed budget on the oligo-recovery nofail stack, with exact-sequence MSA paths reused across shards | Launch only if the v2 oligo-recovery nofail stack justifies the assembly simplification |
...
| P1 | `server_attack_protenix_terminal_tag_seed101_105` | `yang_terminal_tag_cleanup_v1_server_attack` | `runs/server_attack_protenix_terminal_tag_seed101_105/` | `casp16_server_protein_v1` | Slurm job `810719` running; `2026-07-06 16:32 CDT` check had 98 `seed_101` CIFs, 89 `seed_102` CIFs, and 0 for `seed_103..105` | Five fixed seeds on the current best terminal-tag cleanup strategy, selected by predeclared confidence-only policy |
...
| P11 | `server_v2_protenix_yang_oligo_sequence_stoich_low_complexity_large_fallback_seed101` | `yang_oligo_sequence_stoich_low_complexity_large_fallback_v1` | `runs/server_v2_protenix_yang_oligo_sequence_stoich_low_complexity_large_fallback_seed101/` | `casp16_server_protein_v2_aliasfix` | Slurm job `810938` running; `2026-07-06 16:32 CDT` check had 18/165 CIFs | Current strongest v2 `dev_fixed` stack: oligo sequence recovery, token-safe stoich, low-complexity cleanup, fallback; 165 jobs, 0 over-token |
| P12 | `server_v2_attack_oligo_recovery_nofail_msa_reuse_protenix5_seed101_105` | `yang_oligo_sequence_stoich_low_complexity_large_fallback_v1_msa_reuse_server_attack_protenix5` | `runs/server_v2_attack_oligo_recovery_nofail_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` pending on `afterany:810938` | Five fixed seeds on the strongest no-over-token v2 stack, with 268/268 exact-sequence MSA paths reused, selected by confidence-only policy |
| P13 | `server_v2_protenix_yang_oligo_sequence_stoich_hydrophobic_leader_nofail_msa_reuse_seed101` | `yang_oligo_sequence_stoich_low_complexity_hydrophobic_leader_large_fallback_v1_msa_reuse` | `runs/server_v2_protenix_yang_oligo_sequence_stoich_hydrophobic_leader_nofail_msa_reuse_seed101/` | `casp16_server_protein_v2_aliasfix` | Slurm job `811754` pending on `afterany:811751` | Single-seed hydrophobic-leader cleanup on T0240/T1210/T1240 alias jobs; 8 changed sequences, 0 over-token, 260/268 MSA paths reused |
...
older coverage/stoich nofail input.
...
| P24 | `protenix25_nofail` attack tier | budget JSON and shard TSV updated to the MSA-reused oligo-recovery nofail artifact, not queued; merge path implemented | If the no-over-token v2 stack wins, spend 25 seeds on runnable jobs without repeating MSA search in every shard | Launch only after the v2 nofail dev row and `protenix5` evidence justify the GPU-hours; score only via merged run |
