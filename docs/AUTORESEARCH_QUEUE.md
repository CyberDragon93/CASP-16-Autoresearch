...
| P25 prepared | `casp16_server_attack_protenix25_scoreable_input_repair` | `casp16_server_protein_v2_aliasfix` | prepared, not queued; target+seed manifest has 30 rows, reuses the six running P17 seed101-105 target shards, adds 24 deferred seed106-125 specs, and `preflight-runs` reports `30/30 ok` with complete MSA reuse and 0 stale paths | This is the winner-like 25-candidate successor to P17 on the repaired 79-target scoreable input | Submit seed106-125 only if P17 scores as candidate-limited broad signal; otherwise choose D6a/O5/P27 from the post-P17 decision matrix |
...
| P25 | `protenix25_scoreable_input_repair` attack tier | budget JSON, target+seed shard TSV, readiness TSV, and 24 deferred seed106-125 run specs prepared for the P17 79-target repaired input; preflight `30/30 ok` | If P17 is candidate-limited, spend 25 seeds on the repaired locally scoreable jobs without repeating MSA search or reverting to the stale 74-target input | Do not submit before P17 merge/score; score only via merged run with `--merged-candidate-count 25` readiness |
