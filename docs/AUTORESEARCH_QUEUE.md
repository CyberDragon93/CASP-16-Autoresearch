...
| P14 | `server_v2_attack_scoreable_oligo_first_phase_alias_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | pending next v2 candidate; `run-next --dry-run` is blocked by running P13 | Same 74-job scoreable set and same five-candidate budget as P13, but fixes phase-alias stoichiometry so `H0220/H1220/H2220` are recovered protein `A1B4` jobs; preflights at 141/141 exact-sequence MSA paths, 0 missing, 0 stale | Run after P13 completes, fails, or is explicitly superseded; this replaces the stale oligo-first row |
...
| P13 | `server_v2_attack_scoreable_oligo_first_phase_alias_msa_reuse_protenix5_seed101_105` | `scoreable_target_subset_oligo_first_phase_alias_v1_nofail_server_attack_protenix5` | `runs/server_v2_attack_scoreable_oligo_first_phase_alias_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | pending behind running P12; MSA preflight `141/141` reused, `0` missing, 50 exact `protein_oligo` tasks first | Same five fixed seeds and same 74 scoreable jobs as P12, but fixes `H0220/H1220/H2220` phase-alias stoichiometry to `A1B4` before spending the retry budget |
