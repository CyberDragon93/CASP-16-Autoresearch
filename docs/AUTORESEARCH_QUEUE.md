...
| P13 | `server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` running on `c636-072`; `2026-07-06 18:08 CDT` check had 13/74 seed-101 CIFs, after completing long targets `T1210`, `T1249V1`, `T1269`, and `T1280` | Compute-saving five-candidate attack on the current strongest v2 nofail stack: filters prediction inputs to 74/165 jobs with at least one locally available reference alias and reuses 141/141 exact-sequence MSA paths | Monitor until all five declared seeds complete; score only as attack tier with confidence selector |
...
| target_lab | `811918` | `targetlab_protenix_yang_antibody_fv_seed101` | target_lab only | complete on `c620-142`; 8/8 CIFs and confidence summaries | Eight full-MSA/template Fv-only antibody-antigen jobs from `yang_antibody_fv_fragment_inputs_v1`; H0233/H1233 have the strongest antigen-antibody pair-confidence signal; never ranked |
...
| P1 | `server_attack_protenix_terminal_tag_seed101_105` | `yang_terminal_tag_cleanup_v1_server_attack` | `runs/server_attack_protenix_terminal_tag_seed101_105/` | `casp16_server_protein_v1` | Slurm job `810719` running; `2026-07-06 17:56 CDT` check had CIF counts `98/98/47/0/0` for seeds `101..105` | Five fixed seeds on the current best terminal-tag cleanup strategy, selected by predeclared confidence-only policy |
...
| target_lab | `811918` | `targetlab_protenix_yang_antibody_fv_seed101` | `runs/targetlab_protenix_yang_antibody_fv_seed101/` | target_lab only | complete; 8/8 structures and confidence summaries generated | Eight full-MSA/template Fv-only antibody-antigen jobs; H0233/H1233 confidence-positive diagnostic for O5, never ranked |
...
| done | `yang_antibody_fv_fragment_inputs_v1` | target-lab run `811918` complete; 8/8 CIFs | Fv-only changed-target jobs are useful for fast antibody assembly diagnosis; H0233/H1233 are the clearest confidence-positive cases | Not server-ranked; keep separate from full-set claims |
