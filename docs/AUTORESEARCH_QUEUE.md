...
| deferred | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_seed101` | `casp16_server_protein_v2_aliasfix` | deferred:behind_phase_alias_attack; 169 jobs, 0 over-token, 269/276 protein chains have cached MSA; the 7 missing chains collapse to 4 unique sequences now isolated in pending warmup `server_v2_domain_sequence_recovery_msa_warmup_seed101` | Reference-gap triage found protein-domain targets misparsed as DNA or empty jobs (`T1276/T1228V1/T1239V1/T2276` class), but it is a single-seed input-repair ablation | Re-enable after the phase-alias scoreable attack or after the 4-sequence warmup materializes fresh MSA |
...
| P25 | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_seed101` | deferred dev-fixed run spec plus pending 4-task MSA warmup `server_v2_domain_sequence_recovery_msa_warmup_seed101` | Repairs domain inputs that reference-gap triage exposed as protein-vs-DNA/empty parsing failures while keeping the strongest v2 nofail stack under the token limit | Do not queue multi-seed until the phase-alias attack is handled and the 4 unique fresh-MSA sequences are materialized or deliberately accepted as fresh work |
