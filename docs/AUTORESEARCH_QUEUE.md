...
| P1 | Run `server_attack_protenix_terminal_tag_seed101_105` | separate from `dev_fixed` | Slurm job `810719` running; `2026-07-06 20:16 CDT` check had CIF counts `98/98/98/48/0` for seeds `101..105`; partial diagnostic score is weak: domain fixed mean `0.044437`, oligo `0.000000` | Winner-level server comparison should not pretend one seed/sample is enough, but this older v1 terminal-tag path is not beating the v2 scoreable signal | Let the running job finish if it keeps producing, but do not scale terminal-tag to 25 seeds; prioritize v2 scoreable P13/P14/P15 |
...
| P13 | `server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` running on `c636-072`; `2026-07-06 20:20 CDT` check still has 33 seed101 CIFs and is on large target `H0272` as item 34/74; `sstat` shows the batch step still active | Compute-saving five-candidate attack on the current strongest v2 nofail stack: filters prediction inputs to 74/165 jobs and reuses 141/141 exact-sequence MSA paths; exact H-oligo signal is now real: H0223 QSglob `0.591`, H0225 `0.270`, H0233 `0.221` | Keep running for now; P14 exists because this schedule reaches large blockers too early. Score only after all five declared seeds complete, or explicitly supersede if H0272 stalls for much longer |
| P14 | `server_v2_attack_scoreable_oligo_size_first_phase_alias_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | pending next v2 candidate; auto-advance Slurm wrapper `812202` is queued with `afterany:811751`, and current `run-next --dry-run` is blocked by running P13 | Same 74-job scoreable set and same five-candidate budget as P13, fixes phase-alias stoichiometry so `H0220/H1220/H2220` are recovered protein `A1B4`, and sorts exact oligo jobs by token count so 44 smaller oligo jobs run before the 2515-2535 token blockers | Run after P13 completes, fails, or is explicitly superseded; this replaces both stale oligo-first rows |
| P15 | `server_v2_attack_scoreable_antibody_fv_oligo_size_first_phase_alias_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | pending behind P14; complete MSA reuse using global cache plus target-lab Fv MSA source | Same 74-job scoreable set and five-candidate budget as P14, but trims sequence-detected antibody constant regions on 12 H targets after target-lab Fv evidence; exact oligo jobs remain size-sorted | Run only after P14 evidence or explicit supersede; risky O5 branch because target-lab DockQ is not server QSglob |
...
| P1 | `server_attack_protenix_terminal_tag_seed101_105` | `yang_terminal_tag_cleanup_v1_server_attack` | `runs/server_attack_protenix_terminal_tag_seed101_105/` | `casp16_server_protein_v1` | Slurm job `810719` running; `2026-07-06 18:44 CDT` check had CIF counts `98/98/81/0/0` for seeds `101..105` | Five fixed seeds on the current best terminal-tag cleanup strategy, selected by predeclared confidence-only policy |
...
| P12 | `server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105` | `scoreable_target_subset_v1_oligo_recovery_nofail_server_attack_protenix5` | `runs/server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` running on `c636-072`; `2026-07-06 18:44 CDT` check had 24/74 seed-101 CIFs and no later seeds | Five fixed seeds on the strongest no-over-token v2 stack, filtered to 74 locally scoreable jobs, with 141/141 exact-sequence MSA paths reused and confidence-only selection |
| P13 | `server_v2_attack_scoreable_oligo_size_first_phase_alias_msa_reuse_protenix5_seed101_105` | `scoreable_target_subset_oligo_size_first_phase_alias_v1_nofail_server_attack_protenix5` | `runs/server_v2_attack_scoreable_oligo_size_first_phase_alias_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | pending behind running P12; auto-advance wrapper `812202` should pick it after job `811751`; MSA preflight `141/141` reused, `0` missing, exact oligo tasks first and size-sorted | Same five fixed seeds and same 74 scoreable jobs as P12, fixes `H0220/H1220/H2220` phase-alias stoichiometry to `A1B4`, and moves 2515-2535 token exact-oligo blockers after 44 smaller exact oligo jobs |
| P14 | `server_v2_attack_scoreable_antibody_fv_oligo_size_first_phase_alias_msa_reuse_protenix5_seed101_105` | `scoreable_antibody_fv_oligo_size_first_phase_alias_v1_server_attack_protenix5` | `runs/server_v2_attack_scoreable_antibody_fv_oligo_size_first_phase_alias_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | pending behind P13; MSA preflight `141/141` reused, `0` missing, using target-lab Fv MSA source | Tests sequence-only antibody constant trimming on top of the phase-alias size-first attack; changed H0222/H0223/H0225/H0233 phase families, but stays behind the safer P13 scheduler |
...
| P26 | `scoreable_antibody_fv_oligo_size_first_phase_alias_v1` | run spec queued, not launched | Promotes the Fv target-lab signal into an automatic scoreable full-benchmark attack branch without hand-picking targets | Launch only after size-first phase-alias evidence; stop if QSglob rejects the trimmed antibody branch |
