...
| --- | --- | --- | --- | --- | --- |
| P0 | Fix scoreability before more compute | `casp16_server_protein_v2_aliasfix` | current v2 diagnostic has only 13/71 domain OK targets and 8/104 oligo OK targets; only one oligo target has nonzero QSglob; all current oligo OK rows are `sequence_lookup` fallback matches, not exact `*O` matches; reference-gap worklist has 40 existing predictions waiting on native/reference mapping | Required before extra seeds can become server-leaderboard progress | Prioritize reference recovery, missing-prediction coverage, exact oligo artifacts, and target-agnostic QSglob assembly mapping; do not launch 25-seed shards before the running `protenix5` scoreable attack finishes and this gate is rechecked |
| P1 | Run `server_attack_protenix_terminal_tag_seed101_105` | separate from `dev_fixed` | Slurm job `810719` running; `2026-07-06 18:44 CDT` check had CIF counts `98/98/81/0/0` for seeds `101..105` | Winner-level server comparison should not pretend one seed/sample is enough | Monitor GH200 job, then score with `protenix_confidence_v1` only after all declared seeds are present; partial rows stay unranked |
...
| P13 | `server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` running on `c636-072`; `2026-07-06 18:44 CDT` check had 24/74 seed-101 CIFs and no later seeds | Compute-saving five-candidate attack on the current strongest v2 nofail stack: filters prediction inputs to 74/165 jobs with at least one locally available reference alias and reuses 141/141 exact-sequence MSA paths; seed-101 diagnostic has strong exact-domain positives, while exact-gated oligo remains missing until Protenix reaches exact oligo tasks | Monitor until all five declared seeds complete; score only as attack tier with confidence selector |
| P14 | `server_v2_attack_scoreable_oligo_first_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | pending successor; `run-next --dry-run` is blocked by running P13 | Same 74-job scoreable set and same five-candidate budget as P13, but `scoreable_target_subset_oligo_first_v1` moves all 50 exact `protein_oligo` jobs to the front and run-spec preflights at 141/141 exact-sequence MSA paths, 0 missing, 0 stale | Do not submit while P13 is running; use if P13 stalls before exact oligo artifacts or if a clean retry is needed |
...
| pending | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_seed101` | `casp16_server_protein_v2_aliasfix` | pending single-seed run spec; 169 jobs, 0 over-token, 269/276 protein chains have cached MSA; min cache reuse floor is 0.97 | Reference-gap triage found protein-domain targets misparsed as DNA or empty jobs (`T1276/T1228V1/T1239V1/T2276` class); this composes domain sequence recovery with the current v2 nofail stack without changing benchmark rules | Run only after active scoreable attack decisions; treat as a single-seed input-repair ablation, not a multi-seed promotion |
...
| P13 | `server_v2_attack_scoreable_oligo_first_msa_reuse_protenix5_seed101_105` | `scoreable_target_subset_oligo_first_v1_oligo_recovery_nofail_server_attack_protenix5` | `runs/server_v2_attack_scoreable_oligo_first_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | pending behind running P12; MSA preflight `141/141` reused, `0` missing, 50 exact `protein_oligo` tasks first | Same five fixed seeds and same 74 jobs as P12, reordered to get exact oligo artifacts before domain-heavy feedback |
...
| P24 | `protenix25_scoreable_nofail` attack tier | budget JSON and shard TSV created, not queued; merge path already available | If the scoreable no-over-token v2 stack wins, spend 25 seeds on locally scoreable jobs without repeating MSA search in every shard | Launch only after the running scoreable `protenix5` evidence justifies the GPU-hours; score only via merged run |
| P25 | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_seed101` | pending dev-fixed run spec | Repairs domain inputs that reference-gap triage exposed as protein-vs-DNA/empty parsing failures while keeping the strongest v2 nofail stack under the token limit | Do not queue multi-seed until the 7 fresh-MSA chains are materialized or deliberately accepted as fresh work |
