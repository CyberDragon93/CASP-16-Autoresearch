...
| P13 | `server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` running on `c636-072`; `2026-07-06 18:40 CDT` check had 24/74 seed-101 CIFs and no later seeds | Compute-saving five-candidate attack on the current strongest v2 nofail stack: filters prediction inputs to 74/165 jobs with at least one locally available reference alias and reuses 141/141 exact-sequence MSA paths; seed-101 diagnostic has strong domain positives but oligo positives are still sequence-lookup fallback matches | Monitor until all five declared seeds complete; score only as attack tier with confidence selector |
...
| P1 | `server_attack_protenix_terminal_tag_seed101_105` | `yang_terminal_tag_cleanup_v1_server_attack` | `runs/server_attack_protenix_terminal_tag_seed101_105/` | `casp16_server_protein_v1` | Slurm job `810719` running; `2026-07-06 18:25 CDT` check had CIF counts `98/98/67/0/0` for seeds `101..105` | Five fixed seeds on the current best terminal-tag cleanup strategy, selected by predeclared confidence-only policy |
