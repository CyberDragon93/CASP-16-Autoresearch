...
| P13 | `server_v2_attack_scoreable_oligo_recovery_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm job `811751` still running on `c636-072`; it remains stuck at 36 seed101 CIFs while forwarding large `H1220`; last useful diagnostic had domain mean `0.099576`, oligo mean `0.028394`, and nonzero rows `H1202=0.924`, `H0272=0.428`, `H1204=0.421` | Confirms exact H-oligo QSglob can move, but also proves the monolithic Protenix target loop is the bottleneck once MSA is cached | Let it finish if the scheduler keeps it alive, but do not queue another monolithic successor; target-sharded P14 is now the main path |
| P14 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | six GH200 jobs `812239..812244` submitted; all pending by priority, not dependency; each shard is rank-ineligible until merged | Same 74-job scoreable phase-alias input, same five-candidate budget, same `protenix_confidence_v1` selector, and 100% MSA reuse, but split into 6 target-disjoint balanced shards around 11.5k token-estimate each so `H0258/H1258/H2258/H0220/H1220/H2220` no longer block every other target | When all six finish, run `merge-shards --allow-target-shards --merged-input-json strategies/scoreable_target_subset_oligo_size_first_phase_alias_v1/casp16_server_protein_v2_aliasfix/inputs.json`, then score the merged attack row |
...
| P13 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `target_shards_scoreable_size_balanced_v1_server_attack_protenix5` | `strategies/target_shards_scoreable_size_balanced_v1/casp16_server_protein_v2_aliasfix/` and `runs/server_v2_attack_scoreable_size_balanced_shard*/` | `casp16_server_protein_v2_aliasfix` | Slurm jobs `812239..812244` submitted; each shard preflight has complete MSA reuse and is rank-ineligible until merge | Same five fixed seeds and same 74 scoreable phase-alias jobs as the superseded monolithic P13, but target-disjoint balanced shards prevent one 2500-token complex from serially blocking the full candidate budget |
