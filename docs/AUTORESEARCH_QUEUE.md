...
| --- | --- | --- | --- | --- | --- |
| P0 | Fix scoreability before more compute | `casp16_server_protein_v2_aliasfix` | current v2 diagnostic has only 13/71 domain OK targets and 8/104 oligo OK targets; only one oligo target has nonzero QSglob | Required before extra seeds can become server-leaderboard progress | Prioritize reference recovery, missing-prediction coverage, and target-agnostic QSglob assembly mapping; do not launch 25-seed shards before the running `protenix5` scoreable attack finishes and this gate is rechecked |
