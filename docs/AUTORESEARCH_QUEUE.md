...
| --- | --- | --- | --- | --- | --- |
| P0 | Fix scoreability before more compute | `casp16_server_protein_v2_aliasfix` | P14 completed and scored, but the full fixed-set row still had 5 available-reference targets as `missing_prediction`; P16 consensus replay did not improve the score | Required before extra seeds can become server-leaderboard progress | P17 input-repair shards are now the active branch; do not launch 25-seed shards before P17 is merged/scored |
...
| P17 running | `server_v2_attack_scoreable_input_repair_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | Slurm jobs `812765..812770` running; 79/79 locally scoreable targets covered, complete MSA reuse, rank-ineligible until merged | Repairs `T1212`, `T1239V2`, `T1249V2O`, `T1269V1O`, and `T2249V2O` without changing the locked benchmark | Merge/score P17 before any 25-seed, v4, P27a, or O5 launch |
...
| P17 running | `server_v2_attack_scoreable_input_repair_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `target_shards_scoreable_input_repair_size_balanced_v1_server_attack_protenix5` | `strategies/scoreable_target_subset_input_repair_v1/`, `strategies/target_shards_scoreable_input_repair_size_balanced_v1/`, and `runs/server_v2_attack_scoreable_input_repair_size_balanced_shard*/` | `casp16_server_protein_v2_aliasfix` | Slurm jobs `812765..812770` running; six shards are 100% MSA-cache covered, rank-ineligible, and together cover 79/79 locally scoreable targets | Same five-candidate budget as P14, but repairs `T1212`, `T1239V2`, `T1249V2O`, `T1269V1O`, and `T2249V2O`; merge/score only after all declared candidates are present |
...
79-target repaired input, so it must be rebuilt or retargeted after P17 before
any 25-seed launch.
...
| P24 | `protenix25_scoreable_nofail` attack tier | budget JSON, target+seed shard TSV, readiness TSV, and 24 deferred seed106-125 run specs prepared for the pre-P17 74-target input | If the repaired scoreable stack wins, spend 25 seeds on locally scoreable jobs without repeating MSA search in every shard or starting each shard on large blockers | Rebuild/retarget after P17 evidence justifies the GPU-hours; score only via merged run with `--merged-candidate-count 25` readiness |
...
| P27 | `casp16_server_attack_msa_model_diversity_v1` | design JSON created; P27a default-params model/config variant prepared and deferred | Public MULTICOM4/QA4 clues point to diverse MSA/model pools plus QA when a single input stack is valid but weak | Do not submit before P17 scoring; no MSA-disabled toy setting, no hidden per-target variant choice, and broader MSA variants still need automatic preflightable inputs |
