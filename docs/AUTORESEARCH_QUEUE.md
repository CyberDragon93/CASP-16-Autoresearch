...
| --- | --- | --- | --- | --- | --- |
| P0 | Fix scoreability before more compute | `casp16_server_protein_v2_aliasfix` | latest rank-ineligible seed101 diagnostic has 24/71 exact domain OK and 8/104 exact H-oligo OK; H0223/H0225/H0233/H0222/H0227 are nonzero QSglob, so exact oligo scoring is now useful but still coverage-limited; reference-gap worklist has 40 existing predictions waiting on native/reference mapping | Required before extra seeds can become server-leaderboard progress | Let the running scoreable `protenix5` job continue, prioritize reference recovery and exact oligo coverage, and do not launch 25-seed shards before the running attack is scored or explicitly superseded |
...
| P14 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | six GH200 jobs `812239..812244` running; `2026-07-07 04:51 CDT` replay-safe `finish-shards` reports `311/370` candidates, `59` missing, `21/74` target tasks complete, `0/6` shards complete, `ready=false`, and all six shards remain active; log scan is clean; each shard is rank-ineligible until merged | Same 74-job scoreable phase-alias input, same five-candidate budget, same `protenix_confidence_v1` selector, and 100% MSA reuse, but split into 6 target-disjoint balanced shards around 11.5k token-estimate each so `H0258/H1258/H2258/H0220/H1220/H2220` no longer block every other target; current wall time is Protenix forward on large targets, not MSA search | Wait, then rerun replay-safe `finish-shards`; when readiness is true it will merge, register P16 consensus replay, score, and refresh the leaderboard in one audited step |
...
| P18 | `casp16_server_attack_protenix25_scoreable_nofail` | `casp16_server_protein_v2_aliasfix` | prepared, not queued; target+seed shard manifest has 30 rows, reusing the six P14 seed101-105 target shards and adding 24 deferred seed106-125 specs; refreshed `preflight-runs` reports `30/30 ok`, complete MSA reuse, and 0 stale paths | Winner-like compute is likely more than five candidates; this is the scoreable-target 25-seed successor to the running `protenix5` attack without repeating stale H0220 A1B1, early large-target blockers, or the already submitted first seed block | Launch only after P14 is merged/scored or explicitly superseded; rerun `preflight-runs`, then use `check-shards --candidate-count 5 --merged-candidate-count 25` and score only the merged 25-candidate row |
...
| design P27 | `casp16_server_attack_msa_model_diversity_v1` | post-P14 server benchmark, likely `casp16_server_protein_v2_aliasfix` unless v4 is selected first | non-executable budget design recorded in `attack_budgets/casp16_server_attack_msa_model_diversity_v1.json`; selector hook `diversity_confidence_consensus_v1` is implemented; P27a below is the first concrete model/config variant | Reproduces the MULTICOM4/QA4-style lesson: diverse MSA/model generation plus QA, with real MSA/template settings, rather than just turning one Protenix input through more seeds | Build broader MSA/model variants only if P14 and P27a evidence show that model/config diversity is worth more than seed scaling, reference recovery, or input repair |
| P27a | `server_v2_attack_scoreable_defaultparams_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | prepared, not submitted; budget JSON and shard TSV are checked in; all six run specs are `deferred:await_p14_score`, rank-ineligible, `server_attack`, and batch preflight is `6/6 ok` with complete MSA reuse | First executable model/config diversity probe: same P14 scoreable target shards, seeds `101..105`, sample count, real MSA/template, and selector, but flips only `use_default_params:false -> true` | Launch only after P14 is merged/scored and the decision matrix selects model/config diversity; keep as a separate attack row, never merge into P14 |
| deferred | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_after_warmup_seed101` | `casp16_server_protein_v2_aliasfix` | `deferred:await_p14_score`; 169 jobs, 0 over-token, refreshed D6a preflight is `1/1 ok` with complete exact-sequence MSA reuse (`276/276` protein chains, 0 stale) in `diagnostics/msa_cache/domain_sequence_recovery_after_warmup_preflight.tsv` | Reference-gap triage found protein-domain targets misparsed as DNA or empty jobs (`T1276/T1228V1/T1239V1/T2276` class), but it is a single-seed input-repair ablation | Queue only after P14 is merged/scored and the decision matrix selects D6a |
...
| P14 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `target_shards_scoreable_size_balanced_v1_server_attack_protenix5` | `strategies/target_shards_scoreable_size_balanced_v1/casp16_server_protein_v2_aliasfix/` and `runs/server_v2_attack_scoreable_size_balanced_shard*/` | `casp16_server_protein_v2_aliasfix` | Slurm jobs `812239..812244` running; latest readiness is `311/370` candidates, `59` missing, `21/74` complete tasks, and `ready=false`; each shard preflight has complete MSA reuse and is rank-ineligible until merge | Same five fixed seeds and same 74 scoreable phase-alias jobs as the superseded monolithic run, but target-disjoint balanced shards prevent one 2500-token complex from serially blocking the full candidate budget |
| P15 | `server_v4_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `target_shards_scoreable_size_balanced_v1_server_attack_protenix5` | `strategies/target_shards_scoreable_size_balanced_v1/casp16_server_protein_v4_refmap/` and `runs/server_v4_attack_scoreable_size_balanced_shard*/` | `casp16_server_protein_v4_refmap` | prepared only; six run specs are `deferred:await_p14_score`, rank-ineligible, and `diagnostics/msa_cache/protenix5_v4_scoreable_target_run_preflight.tsv` is `6/6 ok` with complete MSA reuse | Same five-candidate scoreable attack on the v4 refmap benchmark, adding the audited `T1278/T2278` scoreable jobs; launch only after P14 is scored and report as v4, not v2 |
...
| P25 | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_after_warmup_seed101` | `deferred:await_p14_score` dev-fixed run spec with complete MSA reuse after completed 4-task warmup `server_v2_domain_sequence_recovery_msa_warmup_seed101` | Repairs domain inputs that reference-gap triage exposed as protein-vs-DNA/empty parsing failures while keeping the strongest v2 nofail stack under the token limit | Do not queue multi-seed until the phase-alias attack is handled; run this single-seed ablation only if the post-P14 decision matrix selects D6a |
| P27 | `casp16_server_attack_msa_model_diversity_v1` | design JSON created; P27a default-params model/config variant prepared and deferred | Public MULTICOM4/QA4 clues point to diverse MSA/model pools plus QA when a single input stack is valid but weak | Do not submit before P14 scoring; no MSA-disabled toy setting, no hidden per-target variant choice, and broader MSA variants still need automatic preflightable inputs |
