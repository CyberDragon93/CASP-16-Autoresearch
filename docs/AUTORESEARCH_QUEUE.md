...
| P14 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | six GH200 jobs `812239..812244` running; `2026-07-07 02:05 CDT` readiness is still `189/370` candidates, `ready=false`, no OOM/traceback/error hits, and stderr tails show active seed103 large-target forward work on 2285-2535 token complexes; each shard is rank-ineligible until merged | Same 74-job scoreable phase-alias input, same five-candidate budget, same `protenix_confidence_v1` selector, and 100% MSA reuse, but split into 6 target-disjoint balanced shards around 11.5k token-estimate each so `H0258/H1258/H2258/H0220/H1220/H2220` no longer block every other target | Wait, then rerun `check-shards`; when all six finish, run `merge-shards --allow-target-shards --merged-input-json strategies/scoreable_target_subset_oligo_size_first_phase_alias_v1/casp16_server_protein_v2_aliasfix/inputs.json`, then score the merged attack row |
