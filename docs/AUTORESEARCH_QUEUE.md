...
Current live P25 gate, checked `2026-07-07 18:14 CDT`: `ready=false`,
`compatible=true`, `1725` observed candidates, `345` shard-level candidates
missing, and `312` full 25-candidate slots missing. Fifty targets are now
complete at the full 25-candidate budget, but the merged P25 row is not
scoreable yet.
Slurm has all remaining P25 shards running, and the CPU closeout watcher
`814295` is running on `gg`. Do not submit any branch in this section until the
complete P25 row is merged and scored.
Current running shards show normal large-complex inference, not repeated MSA
recomputation. The latest P25 error logs are empty, `zero_output_shard_count=0`,
and `run-next --dry-run` reports `no_pending_runs`.
...
Follow-up health check, `2026-07-07 18:14 CDT`: P25 error logs are empty, the
newest CIF/JSON mtimes are still advancing inside running shards, with recent
writes through 18:03 CDT, and `zero_output_shard_count=0`. The current tail is
running large-complex inference, not observed execution failure, so keep waiting
for declared candidates rather than scoring partial output.
...
is selected. Latest audit `2026-07-07 18:14 CDT`: all four are still
launch-ready after complete P25 selection. A stale deferred status label is not
itself a blocker; missing run specs or non-`ok` preflight rows are blockers.
...
| 6 | P25 and P27b are complete, valid, and still flat while score loss plausibly comes from MSA depth/pairing rather than input aliases | P28a-style production MSA mode probe with `msa_server_mode=colabfold` or equivalent precomputed ColabFold/MMseqs A3M paths | Reproduces the MULTICOM/QA lesson using a real MSA construction change instead of a toy no-MSA shortcut | Create new run specs only after the MSA paths/runtime are proven; keep `use_msa=true`, `use_template=true`, fixed seeds, and no per-target score tuning |
...
The Lane B deferred-hit review is now recorded in
`diagnostics/reference_gap/casp16_server_protein_v5_lane_b_deferred_hit_review.tsv`.
It reviews all five queued deferred-hit families and rejects them for refmap
promotion: `10br` is only a short HtrA fragment, the H0215-family hits do not
...
The Lane E domain search review is recorded in
`diagnostics/reference_gap/casp16_server_protein_v5_lane_e_domain_search_review.tsv`.
It unifies the relaxed90 RCSB evidence for all `16` queued domain families and
`36` target rows. Fifteen families have no relaxed90 sequence hits. The only
hits are `5AQ5`, `4UW7`, and `4UW8` for `T0257/T1257/T2257`, and those are
short tail-fiber domain/template structures rather than the 1263-residue CASP
construct. Lane E should now stay in manual native/provenance search plus
explicit domain-crop mapping; do not run another search-depth-only RCSB loop.
...
The final Lane F probe is recorded in
`diagnostics/reference_gap/rcsb_relaxed90_probe_20260707_lane_f_final_oligo_targets.tsv`:
it checked the remaining `5` oligo rows from isocyanide hydratase-like,
isocyanide hydratase, and Bd3442 families at identity `0.90`, also with `0`
hits and a header-only candidate TSV. Lane F automatic relaxed RCSB sequence
probing is now exhausted across all `13` families / `35` target rows; further
work should be manual native/provenance search and QSglob assembly mapping.
...
| done selector replay | `server_v2_attack_scoreable_input_repair_overlay_msa_reuse_protenix5_seed101_105_ranking_consensus_replay` | `casp16_server_protein_v2_aliasfix` | completed no-GPU replay on the same P17 predictions: domain `0.105372`, oligo `0.116654`, below both original P17 and the older consensus replay | Tests whether Protenix native `ranking_score` plus prediction-only consensus is a shortcut to better model selection | Negative on the 5-candidate baseline; keep P25 ranking replay only as a diagnostic on the larger 25-candidate pool |
...
| P25 submitted | `casp16_server_attack_protenix25_scoreable_input_repair` | `casp16_server_protein_v2_aliasfix` | submitted Slurm jobs `812935..812958` for the 24 seed106-125 target-seed shards; latest live gate at `2026-07-07 18:14 CDT` is `ready=false`, `compatible=true`, all remaining P25 shards running, `1725` observed candidates, `345` shard-level missing, `50/79` full-budget tasks complete, complete MSA reuse, `zero_output_shard_count=0`, and large-complex inference currently active on scoreable targets | This is the winner-like 25-candidate successor to P17 on the repaired 79-target scoreable input | Wait for all jobs to finish, merge with the overlay, then score/leaderboard the complete 25-candidate row; do not score partial output or launch O5b/P27b early |
...
| design P27/P28 | `casp16_server_attack_msa_model_diversity_v1` | P17/P25 repaired-input server benchmark unless post-P25 readout selects refmap first | P25-era design recorded in `attack_budgets/casp16_server_attack_msa_model_diversity_v1.json`; `diversity_confidence_consensus_v1` is implemented, P25 consensus replay is configured in the closeout wrapper, P27b is the prepared model/config child, and `run-spec` now records `msa_server_mode` for a future ColabFold/MMseqs production-MSA child | Reproduces the MULTICOM4/QA4-style lesson: diverse MSA/model generation plus QA, with real MSA/template settings, rather than just turning one Protenix input through more seeds | Launch P27b first only if complete P25 is flat but valid; build P28a true MSA variants only after P25/P27b show model/config or MSA diversity is the right next spend |
...
| P15 | `server_v4_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `target_shards_scoreable_size_balanced_v1_server_attack_protenix5` | `strategies/target_shards_scoreable_size_balanced_v1/casp16_server_protein_v4_refmap/` and `runs/server_v4_attack_scoreable_size_balanced_shard*/` | `casp16_server_protein_v4_refmap` | prepared only; six run specs are `deferred:await_p25_score`, rank-ineligible, and `diagnostics/msa_cache/protenix5_v4_scoreable_target_run_preflight.tsv` is `6/6 ok` with complete MSA reuse | Same five-candidate scoreable attack on the v4 refmap benchmark, adding the audited `T1278/T2278` scoreable jobs; re-evaluate after P25 and report as v4, not v2 |
