...
| P14 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | six GH200 jobs `812239..812244` running; `2026-07-07 02:37 CDT` `finish-shards --dry-run` reports `203/370` candidates, `167` missing, `ready=false`, no OOM/traceback/error hits, and all six shards remain active; each shard is rank-ineligible until merged | Same 74-job scoreable phase-alias input, same five-candidate budget, same `protenix_confidence_v1` selector, and 100% MSA reuse, but split into 6 target-disjoint balanced shards around 11.5k token-estimate each so `H0258/H1258/H2258/H0220/H1220/H2220` no longer block every other target | Wait, then rerun `finish-shards`; when readiness is true it will merge, score, and refresh the leaderboard in one audited step |
...
| P16 | `server_v2_attack_scoreable_size_balanced_msa_reuse_protenix5_seed101_105_consensus_replay` | `casp16_server_protein_v2_aliasfix` | predeclared before P14 score in `attack_budgets/casp16_server_attack_protenix5_consensus_selector_replay.json`; no new GPU work | Replays the same completed P14 prediction pool with `diversity_confidence_consensus_v1` after `selection-qa`, testing the CASP16 winner clue that QA/model selection matters without using references or official scores | After P14 merge and before inspecting target scores, register the replay row, run `selection-qa --run-id`, then score it as a separate `server_attack` row |
