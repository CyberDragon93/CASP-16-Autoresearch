...
| P27 | `casp16_server_attack_msa_model_diversity_v1` | design only | Public MULTICOM4/QA4 clues point to diverse MSA/model pools plus QA when a single input stack is valid but weak | Do not build before P14 scoring; no MSA-disabled toy setting, no hidden per-target variant choice, and no executable attack budget until variant inputs are automatic and preflightable |
