...
| P13 | `server_v2_attack_oligo_recovery_nofail_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | pending, not submitted; token-safe and cache-ready | Stronger five-candidate no-over-token attack with exact-sequence MSA reuse: fixes protein-oligo sequence modality, restores token-safe stoich, applies low-complexity cleanup and fallback, 165 jobs, 0 over-token, 268/268 protein-chain MSA paths reused | Let the running v2 dev row finish first; `run-next --dry-run` should select this after the active v2 run clears |
...
| P12 | `server_v2_attack_oligo_recovery_nofail_msa_reuse_protenix5_seed101_105` | `yang_oligo_sequence_stoich_low_complexity_large_fallback_v1_msa_reuse_server_attack_protenix5` | `runs/server_v2_attack_oligo_recovery_nofail_msa_reuse_protenix5_seed101_105/` | `casp16_server_protein_v2_aliasfix` | pending, not submitted | Five fixed seeds on the strongest no-over-token v2 stack, with 268/268 exact-sequence MSA paths reused, selected by confidence-only policy |
