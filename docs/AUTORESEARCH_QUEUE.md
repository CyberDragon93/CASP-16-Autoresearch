...
Current live P25 gate, checked `2026-07-07 17:30 CDT`: `ready=false`,
`compatible=true`, `1648` observed candidates, `418` shard-level candidates
missing, and `373` full 25-candidate slots missing. Forty-four targets are
...
`QOSMaxJobsPerUserLimit`, and one `gb` `tacc-vscode` job is also running. Do
...
Follow-up health check, `2026-07-07 17:30 CDT`: a keyword scan across the P25
...
running shards, with recent writes through 17:30 CDT. The five zero-output
shards in the readiness JSON correspond to pending jobs behind the queue limit,
not observed execution failures, so keep waiting for declared candidates rather
than scoring partial output.
...
is selected. Latest audit `2026-07-07 17:30 CDT`: all four are still
