...
| deferred | `server_v2_domain_sequence_recovery_oligo_nofail_msa_reuse_after_warmup_seed101` | `casp16_server_protein_v2_aliasfix` | pending behind P14; 169 jobs, 0 over-token, complete exact-sequence MSA reuse after the 4-task warmup materialized the D6a missing sequences (`276/276` protein chains, 0 stale) | Reference-gap triage found protein-domain targets misparsed as DNA or empty jobs (`T1276/T1228V1/T1239V1/T2276` class), but it is a single-seed input-repair ablation | Queue only after the phase-alias scoreable attack is handled or if a current allocation slot is explicitly assigned to D6a scoring |
