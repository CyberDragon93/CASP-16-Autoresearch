...
| P14 | `server_v2_attack_scoreable_size_balanced_shard01..06_msa_reuse_protenix5_seed101_105` | `casp16_server_protein_v2_aliasfix` | six GH200 jobs `812239..812244` running; `2026-07-07 03:21 CDT` replay-safe `finish-shards` reports `255/370` candidates, `115` missing, `ready=false`, and all six shards remain active; log scan is clean; each shard is rank-ineligible until merged | Same 74-job scoreable phase-alias input, same five-candidate budget, same `protenix_confidence_v1` selector, and 100% MSA reuse, but split into 6 target-disjoint balanced shards around 11.5k token-estimate each so `H0258/H1258/H2258/H0220/H1220/H2220` no longer block every other target | Wait, then rerun replay-safe `finish-shards`; when readiness is true it will merge, register P16 consensus replay, score, and refresh the leaderboard in one audited step |
