...
| P1 | Run `server_attack_protenix_terminal_tag_seed101_105` | separate from `dev_fixed` | Slurm job `810719` running; latest check had 98 `seed_101` CIFs and 63 `seed_102` CIFs | Winner-level server comparison should not pretend one seed/sample is enough | Monitor GH200 job, then score with `protenix_confidence_v1` only after all declared seeds are present; partial rows stay unranked |
...
| P1 | `server_attack_protenix_terminal_tag_seed101_105` | `yang_terminal_tag_cleanup_v1_server_attack` | `runs/server_attack_protenix_terminal_tag_seed101_105/` | `casp16_server_protein_v1` | Slurm job `810719` running; latest check had 98 `seed_101` CIFs and 63 `seed_102` CIFs | Five fixed seeds on the current best terminal-tag cleanup strategy, selected by predeclared confidence-only policy |
