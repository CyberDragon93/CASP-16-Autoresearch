...
Checked `2026-07-07 15:08 CDT`: P25 is still incomplete, but the live jobs
look healthy.
...
| observed candidates | `1278` |
| shard-level missing candidates | `780` |
| full 25-candidate slots still missing | `710` |
...
| Local comparison is capped by missing references and QSglob mapping | Versioned refmap work: v4 now has 81/175 refs; v5 queue is lane-based | Lane E relaxed RCSB probe added no promotable rows; all 13 Lane F oligo families now have relaxed90 probe coverage with no accepted references; H1265 input aliases are repaired but reference-blocked | Continue only strict v5 reference lanes while GPU jobs run | Do not patch v2/v4 in place or promote sequence hits without native/reference proof |
