...
1. Finish and score `server_protenix_full_msa_template_seed101`, the first full
   server-target Protenix baseline with real MSA/templates.
2. Run `yang_terminal_tag_cleanup_v1` as the first target-agnostic
   optimized-input reproduction of a Yang-style recipe.
3. Add explicit domain cropping and chain/residue mapping for
   `casp16_server_protein_v1` domain `GDT_TS` scoring.
4. Install or implement a real `QSglob` scorer for server oligos.
5. Improve the reference/domain registry for the 121 server-benchmark targets
   that currently lack a cached reference mapping.
6. Add oligo assembly mapping.
...
8. Add a full server-target inference queue for the 106 Protenix/OpenDDE jobs
   generated by `casp16_server_protein_v1`.
9. Implement strategy experiments inspired by CASP16 winners:
