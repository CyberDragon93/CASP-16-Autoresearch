...
def write_markdown(rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        "# Small Complex Target-Lab Summary",
        "",
        "This summary is diagnostic only and is not a leaderboard artifact.",
        "",
        f"- jobs: {len(rows)}",
        f"- ok: {ok}",
        ...
        "",
        "| job | target | status | total_len | structures | confidence | plddt | ptm | iptm |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in rows:
        lines.append(
            "| {job_name} | {target_id} | {status} | {total_len} | {structure_files} | {confidence_files} | {plddt} | {ptm} | {iptm} |".format(
                **row
            )
        )
...
def main() -> int:
    ...
    rows = [summarize_job(row) for row in manifest_rows]
