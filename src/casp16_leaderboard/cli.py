...
from .leaderboard import collect_local_runs, generate_official_leaderboard, write_coverage_report
from .official import ingest_official_data
from .runs import DEFAULT_PROTENIX_BIN, DEFAULT_PROTENIX_ROOT, create_run_spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run-spec":
        input_json = (args.input_json or (root / "data" / "inputs" / "casp16_all.json")).resolve()
        input_manifest = (args.input_manifest or (root / "data" / "inputs" / "casp16_all.manifest.tsv")).resolve()
