...
- Strategy name: `yang_epitope_tag_cleanup_v1`
...
Some CASP16 server inputs contain obvious N-terminal epitope/His/TEV tags or
C-terminal His tags. Removing those sequence-only expression artifacts may
improve fold and assembly quality without using references, official scores, or
target-specific post hoc choices.
...
- Changed code/scripts: generated strategy input under
  `strategies/yang_epitope_tag_cleanup_v1/casp16_server_protein_v1/`.
- Unchanged fixed budget:
  ...
  - MSA/templates/default Protenix params/cache/fusion/TF32: enabled, matching
    the parent baseline and other queued optimized-input runs
...
- Rank status: pending, queued behind
  `server_protenix_yang_terminal_tag_antibody_fv_cleanup_seed101`.
...
- Artifact path: `runs/server_protenix_yang_epitope_tag_cleanup_seed101/`.
...
`running`; it is ordered after the lower-risk terminal-tag, antibody-Fv, and
combined construct-cleanup ablations.
...
Let the full baseline finish, run the lower-risk queued construct ablations in
order, then use this run to test whether broader epitope/TEV tag cleanup is a
net full-set improvement.
