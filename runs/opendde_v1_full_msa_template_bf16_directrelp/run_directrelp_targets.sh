...
export OPENDDE_DIRECT_RELP=1
export OPENDDE_DIRECT_RELP_CHUNK_ROWS=256
RUN_DIR=/scratch/10992/liaorunlong93/casp16-leaderboard/runs/opendde_v1_full_msa_template_bf16_directrelp
OUT_DIR="$RUN_DIR/predictions/opendde_v1"
: > "$RUN_DIR/logs/stdout.log"
: > "$RUN_DIR/logs/stderr.log"
: > "$RUN_DIR/logs/directrelp_status.tsv"
...
for target in H1258 H1236 T1210 H0272 H1272 H1227; do
  ...
  printf '%s	%s	%s	%s	%s
' "$target" "$status" "$code" "$started" "$finished" >> "$RUN_DIR/logs/directrelp_status.tsv"
